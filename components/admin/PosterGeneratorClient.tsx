...
async function inlineSvgImages(svg: string) {
  ...
  await Promise.all(hrefs.map(async (href) => {
    try {
      ...
      hydratedSvg = hydratedSvg.split(href).join(dataUrl);
    } catch {
      // Keep the remote URL. The SVG download will still work even when raster export cannot inline one image.
    }
  }));
...
async function rasterizeSvg(svg: string, filename: string, format: Exclude<ExportFormat, 'svg'>) {
  ...
  try {
    ...
    const blob = await new Promise<Blob>((resolve, reject) => {
      canvas.toBlob((nextBlob) => nextBlob ? resolve(nextBlob) : reject(new Error('Unable to create image file.')), mime, format === 'jpeg' ? 0.92 : undefined);
    });
...
export default function PosterGeneratorClient({ products }: { products: PosterProduct[] }) {
  ...
  async function downloadPoster(format: ExportFormat) {
    ...
    try {
      if (format === 'svg') {
        saveBlob(new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }), `${baseName}.svg`);
      } else {
        await rasterizeSvg(svg, baseName, format);
      }
      setExportMessage(`Downloaded ${format.toUpperCase()} poster`);
    } catch {
      setExportMessage('PNG/JPEG export could not load one product image. Try SVG, or use images hosted with public CORS access.');
    } finally {
      setExporting(null);
    }
  }
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      <div className="space-y-4">
        ...
        <div>
          <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-slate-500">Platform caption style</label>
          <select value={platformKey} onChange={(event) => setPlatformKey(event.target.value as PlatformKey)} className="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-orange-300 focus:ring-4 focus:ring-orange-100">
            {PLATFORMS.map((platform) => <option key={platform.key} value={platform.key}>{platform.name} — {platform.hint}</option>)}
          </select>
          <p className="mt-2 text-xs text-slate-500">{selectedPlatform.hint}</p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Download poster</p>
          <div className="mt-3 flex flex-wrap gap-3">
            <button disabled={Boolean(exporting)} onClick={() => downloadPoster('png')} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 py-3 text-sm font-bold text-white transition hover:bg-orange-400 disabled:cursor-not-allowed disabled:opacity-60"><Download className="h-4 w-4" /> {exporting === 'png' ? 'Creating PNG…' : 'Download PNG'}</button>
            <button disabled={Boolean(exporting)} onClick={() => downloadPoster('jpeg')} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-slate-950 px-5 py-3 text-sm font-bold text-white transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60"><Download className="h-4 w-4" /> {exporting === 'jpeg' ? 'Creating JPEG…' : 'Download JPEG'}</button>
            <button disabled={Boolean(exporting)} onClick={() => downloadPoster('svg')} className="inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 px-5 py-3 text-sm font-bold text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">SVG backup</button>
          </div>
          {exportMessage ? <p className="mt-3 text-xs font-semibold text-slate-600">{exportMessage}</p> : null}
        </div>
      </div>
