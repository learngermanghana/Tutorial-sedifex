...
type PosterTemplate = { key: TemplateKey; name: string; description: string; accent: string; accentSoft: string; dark: string; light: string; theme: 'light' | 'dark'; mode?: 'single' | 'collage' };
...
function qrUrl(value: string) { return `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(value)}`; }
function escapeSvg(value: string) { return value.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;'); }
function marketUrl(products: PosterProduct[]) { const firstStore = products[0]?.storeName || ''; return firstStore ? `https://www.sedifexmarket.com/products?search=${encodeURIComponent(firstStore)}` : 'https://www.sedifexmarket.com/products'; }
function activeUrl(product: PosterProduct, products: PosterProduct[], template: PosterTemplate) { return template.mode === 'collage' ? marketUrl(products) : product.productUrl; }
...
function caption(product: PosterProduct, message: string, template: PosterTemplate, products: PosterProduct[], platform: PlatformKey) {
  const url = activeUrl(product, products, template);
  const cta = platformCallToAction(platform, url);
...
function productCardSvg(product: PosterProduct, x: number, y: number, width: number, height: number, template: PosterTemplate) {
  const safeName = escapeSvg(product.name.slice(0, 34)); const safePrice = escapeSvg(product.price || 'Shop now');
...
function buildCollageSvg(products: PosterProduct[], message: string, template: PosterTemplate) {
  const chosen = products.slice(0, 4); const qr = qrUrl(marketUrl(chosen)); const safeMessage = escapeSvg(message);
  const cards = chosen.map((product, index) => { const positions = [{ x: 90, y: 345 }, { x: 555, y: 345 }, { x: 90, y: 755 }, { x: 555, y: 755 }]; const p = positions[index]; return productCardSvg(product, p.x, p.y, 435, 360, template); }).join('');
...
function buildPosterSvg(product: PosterProduct, message: string, template: PosterTemplate, products: PosterProduct[]) {
  ...
  const qr = qrUrl(product.productUrl); const safeName = escapeSvg(product.name); const safePrice = escapeSvg(product.price || 'Shop now'); const safeStore = escapeSvg(product.storeName); const safeMessage = escapeSvg(message); const safeTemplateName = escapeSvg(template.name);
  const background = template.theme === 'dark' ? template.dark : template.accent; const card = template.theme === 'dark' ? '#0f172a' : template.light; const primaryText = template.theme === 'dark' ? '#ffffff' : '#0f172a'; const secondaryText = template.theme === 'dark' ? '#cbd5e1' : '#475569'; const badgeFill = template.theme === 'dark' ? '#111827' : template.accentSoft; const footerFill = template.theme === 'dark' ? template.accent : template.dark;
...
export default function PosterGeneratorClient({ products }: { products: PosterProduct[] }) {
  ...
  function toggleCollageProduct(productId: string) { setSelectedCollageIds((current) => current.includes(productId) ? current.filter((id) => id !== productId) : current.length >= 4 ? [...current.slice(1), productId] : [...current, productId]); }
  async function copyCaption() { await navigator.clipboard.writeText(generatedCaption); setCopyMessage(`Copied ${selectedPlatform.name} caption`); }
  function downloadPoster() { if (!selected) return; const svg = buildPosterSvg(selected, deliveryMessage, selectedTemplate, activeProducts.length ? activeProducts : [selected]); const blob = new Blob([svg], { type: 'image/svg+xml;charset=utf-8' }); const url = URL.createObjectURL(blob); const anchor = document.createElement('a'); anchor.href = url; anchor.download = `${selectedTemplate.mode === 'collage' ? 'sedifex-product-collage' : selected.name.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-${selectedTemplate.key}-poster.svg`; anchor.click(); URL.revokeObjectURL(url); }
