...
const TEMPLATES = [
  { key: 'bigDeals' as const, name: 'Shop Local Big Deals', description: 'Campaign flyer with orange/green Sedifex Market branding', accent: '#ff6a00' },
  { key: 'market' as const, name: 'Classic Product Poster', description: 'Clean single-product marketplace poster', accent: '#ff7a00' },
  { key: 'collage' as const, name: 'Product Collage', description: 'Show 2–4 searched products in one flyer', accent: '#ff7a00' },
];

const PLATFORMS = [
  { key: 'instagram' as const, name: 'Instagram', hint: 'Comment LINK or tap link in bio' },
  { key: 'tiktok' as const, name: 'TikTok', hint: 'Comment LINK for direct order' },
  { key: 'whatsapp' as const, name: 'WhatsApp', hint: 'Direct product link included' },
  { key: 'telegram' as const, name: 'Telegram', hint: 'Clickable link included' },
  { key: 'facebook' as const, name: 'Facebook', hint: 'Clickable link included' },
  { key: 'generic' as const, name: 'Generic', hint: 'Works anywhere' },
];
...
function bigDealsSvg(product: PosterProduct, message: string) {
  const qr = qrUrl(product.productUrl);
  const title = esc(product.name);
  const store = esc(product.storeName);
  const price = esc(product.price || 'Shop now');
  ...
  <ellipse cx="725" cy="1060" rx="250" ry="42" fill="#fff" opacity=".94"/><rect x="545" y="230" width="370" height="570" rx="36" fill="#fff" opacity=".96" filter="url(#s)"/><image href="${product.imageUrl}" x="565" y="255" width="330" height="520" preserveAspectRatio="xMidYMid meet"/>
  <text x="725" y="850" text-anchor="middle" font-size="34" font-weight="900" fill="#fff">${title}</text><text x="725" y="890" text-anchor="middle" font-size="25" font-weight="800" fill="#d1fae5">Sold by ${store}</text>
  <text x="725" y="965" text-anchor="middle" font-size="58" font-weight="950" fill="#fff">${price}</text><text x="725" y="1016" text-anchor="middle" font-size="28" font-weight="800" fill="#fff7ed">${msg}</text>
  <rect x="828" y="920" width="165" height="205" rx="22" fill="#fff"/><image href="${qr}" x="845" y="938" width="132" height="132"/><text x="910" y="1100" text-anchor="middle" font-size="18" font-weight="950" fill="#064e3b">SCAN TO BUY</text>
...
function marketSvg(product: PosterProduct, message: string) {
  const qr = qrUrl(product.productUrl);
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="1080" height="1350" fill="#ff7a00"/><rect x="60" y="60" width="960" height="1230" rx="42" fill="#fff"/><text x="105" y="150" font-size="42" font-weight="900" fill="#ff7a00">Sedifex Market</text><text x="105" y="235" font-size="68" font-weight="950" fill="#0f172a">${esc(product.name)}</text><text x="105" y="285" font-size="32" font-weight="800" fill="#475569">Sold by ${esc(product.storeName)}</text><rect x="105" y="335" width="870" height="515" rx="36" fill="#f8fafc"/><image href="${product.imageUrl}" x="135" y="365" width="810" height="455" preserveAspectRatio="xMidYMid meet"/><rect x="105" y="890" width="870" height="145" rx="30" fill="#fff7ed"/><text x="145" y="955" font-size="56" font-weight="950" fill="#ea580c">${esc(product.price || 'Shop now')}</text><text x="145" y="1005" font-size="30" font-weight="800" fill="#475569">${esc(message)}</text><text x="105" y="1140" font-size="44" font-weight="950" fill="#0f172a">Scan to buy now</text><text x="105" y="1190" font-size="28" font-weight="800" fill="#475569">Verified sellers • Secure checkout</text><image href="${qr}" x="755" y="1080" width="210" height="210"/><rect x="105" y="1260" width="500" height="62" rx="31" fill="#0f172a"/><text x="145" y="1300" font-size="28" font-weight="900" fill="#fff">www.sedifexmarket.com</text></svg>`;
}

function collageSvg(products: PosterProduct[], message: string) {
  ...
  const cards = chosen.map((product, index) => {
    ...
    return `<rect x="${p.x}" y="${p.y}" width="435" height="360" rx="28" fill="#fff"/><rect x="${p.x + 18}" y="${p.y + 18}" width="399" height="200" rx="22" fill="#f8fafc"/><image href="${product.imageUrl}" x="${p.x + 28}" y="${p.y + 28}" width="379" height="180" preserveAspectRatio="xMidYMid meet"/><text x="${p.x + 24}" y="${p.y + 255}" font-size="25" font-weight="900" fill="#0f172a">${esc(product.name.slice(0, 34))}</text><text x="${p.x + 24}" y="${p.y + 290}" font-size="18" font-weight="800" fill="#475569">${esc(product.storeName.slice(0, 32))}</text><text x="${p.x + 24}" y="${p.y + 330}" font-size="30" font-weight="900" fill="#ff7a00">${esc(product.price || 'Shop now')}</text>`;
  }).join('');
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}" viewBox="0 0 ${W} ${H}"><rect width="1080" height="1350" fill="#ff7a00"/><text x="90" y="125" font-size="42" font-weight="900" fill="#fff">Sedifex Market</text><text x="90" y="205" font-size="78" font-weight="950" fill="#fff">Fresh Picks</text><text x="90" y="260" font-size="34" font-weight="800" fill="#fff7ed">Shop verified products online</text>${cards}<rect x="90" y="1160" width="610" height="110" rx="30" fill="#0f172a"/><text x="130" y="1210" font-size="34" font-weight="900" fill="#fff">${esc(message)}</text><text x="130" y="1250" font-size="24" fill="#cbd5e1">Scan QR to shop these products</text><image href="${qr}" x="770" y="1125" width="220" height="220"/></svg>`;
}

function buildSvg(product: PosterProduct, products: PosterProduct[], template: TemplateKey, message: string) {
  if (template === 'bigDeals') return bigDealsSvg(product, message);
  if (template === 'collage') return collageSvg(products, message);
  return marketSvg(product, message);
}
...
export default function PosterGeneratorClientV3({ products }: { products: PosterProduct[] }) {
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
      ...
      <div className="rounded-[32px] border border-slate-200 bg-slate-100 p-5">
        {templateKey === 'bigDeals' ? <div className="mx-auto max-w-[420px] overflow-hidden rounded-[28px] bg-orange-500 text-white shadow-2xl"><div className="grid min-h-[560px] grid-cols-[0.48fr_0.52fr]"><div className="p-5"><p className="text-xl font-black">Sedifex</p><p className="text-xs font-bold">Market</p><h3 className="mt-12 text-3xl font-black leading-tight">Shop Local,<br />Big Deals</h3><p className="mt-3 text-sm font-bold text-orange-100">Trusted Ghana stores, delivered to you.</p><div className="mt-6 space-y-2 text-[10px] font-black text-green-900"><p className="rounded-full bg-yellow-300 px-3 py-2">Trusted Ghana Stores</p><p className="rounded-full bg-yellow-300 px-3 py-2">Secure Checkout</p><p className="rounded-full bg-yellow-300 px-3 py-2">Same-day Delivery</p><p className="rounded-full bg-yellow-300 px-3 py-2">Verified Sellers</p></div></div><div className="bg-green-700 p-5"><div className="mt-16 rounded-3xl bg-white p-4 shadow-2xl">{selected.imageUrl ? <img src={selected.imageUrl} alt={selected.name} className="h-60 w-full object-contain" /> : null}</div><p className="mt-4 text-center text-base font-black">{selected.name}</p><p className="mt-1 text-center text-xs font-bold text-green-100">Sold by {selected.storeName}</p><p className="mt-4 text-center text-3xl font-black">{selected.price || 'Shop now'}</p><div className="mt-4 flex items-center justify-between rounded-2xl bg-white p-3 text-green-900"><span className="text-xs font-black">SCAN TO BUY</span><img src={qrUrl(selected.productUrl)} alt="QR Code" className="h-20 w-20" /></div></div></div><div className="bg-green-900 px-5 py-3 text-xs font-bold">Safe. Simple. • Shop local. Support Ghana. • Fast delivery.</div><div className="bg-orange-600 px-5 py-3 text-xs font-bold">www.sedifexmarket.com <span className="float-right">Powered by Sedifex</span></div></div> : templateKey === 'collage' ? <div className="mx-auto max-w-[420px] overflow-hidden rounded-[32px] bg-orange-500 p-6 text-white shadow-2xl"><p className="text-sm font-semibold uppercase tracking-[0.2em]">Sedifex Market</p><h3 className="mt-3 text-4xl font-black leading-tight">Fresh Picks</h3><div className="mt-6 grid grid-cols-2 gap-3">{activeProducts.slice(0, 4).map((product) => <div key={product.id} className="rounded-3xl bg-white p-3 text-slate-950">{product.imageUrl ? <img src={product.imageUrl} alt={product.name} className="h-32 w-full rounded-2xl bg-slate-100 object-cover" /> : null}<p className="mt-2 truncate text-sm font-black">{product.name}</p><p className="truncate text-xs font-bold text-slate-500">{product.storeName}</p><p className="mt-1 text-sm font-black text-orange-500">{product.price || 'Shop now'}</p></div>)}</div><div className="mt-6 flex items-center justify-between gap-4 rounded-3xl bg-slate-950 p-4"><div><p className="text-lg font-black">Scan to shop</p><p className="mt-1 text-xs text-slate-300">{message}</p></div><img src={qrUrl(marketUrl(activeProducts))} alt="QR Code" className="h-24 w-24 rounded-2xl bg-white p-2" /></div></div> : <div className="mx-auto max-w-[420px] overflow-hidden rounded-[32px] bg-white text-slate-950 shadow-2xl"><div className="bg-orange-500 px-6 py-5 text-white"><p className="text-sm font-semibold uppercase tracking-[0.2em]">Sedifex Market</p><h3 className="mt-3 text-3xl font-black leading-tight">{selected.name}</h3><p className="mt-2 text-sm font-bold text-white/90">Sold by {selected.storeName}</p></div><div className="p-5"><div className="overflow-hidden rounded-3xl bg-slate-100">{selected.imageUrl ? <img src={selected.imageUrl} alt={selected.name} className="h-[360px] w-full object-cover" /> : null}</div><div className="mt-5 rounded-3xl bg-orange-50 p-5"><p className="text-3xl font-black text-orange-600">{selected.price || 'Shop now'}</p><p className="mt-2 text-sm leading-6 text-slate-600">{message}</p></div><div className="mt-6 flex items-center justify-between gap-4"><div><p className="text-xl font-bold">Scan to buy now</p><p className="mt-2 text-sm text-slate-500">Verified sellers • Secure checkout</p></div><img src={qrUrl(selected.productUrl)} alt="QR Code" className="h-28 w-28 rounded-2xl border border-slate-200 bg-white p-2" /></div></div></div>}
      </div>
    </div>
  );
}
